		...
		function edd_listen_for_campay_return() {
			...
			if(isset($_GET['external_reference']) && strstr($_GET['external_reference'], "EDD"))
			{
				...
				if($payment)
				{
    				...
    				if((strtolower($status)=="canceled" || strtolower($status)=="cancelled") || (strtolower($status)=="failed" || strtolower($status)=="failed") )
    				{
